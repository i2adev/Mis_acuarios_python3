...
class FormatoConsumibleController(BaseController):
    """ 
    Controlador base del formulario maestro del formato del proyecto. 
    """
    ...
    def __init__(self, view: FormatoConsumibleDialog |
        ...
        # Atributos
        self._formato_consumible_result = None

        # Llamaos al constructor de la superclase
        super().__init__(view, dao, model)

    def _entity_configuration(self) -> FormatoConsumibleEntity:
    ...
    def _validate_view(self) -> Result:
        ...
        res = FormatoConsumibleValidator.validate_formato_consumible(
            self._view.frame.edit_formato_consumible
        )

        if not res.is_success:
            self._view.frame.edit_formato_consumible.setFocus()
            return res
    ...
    def _get_formato_consumible(self):
        """ Devuelve la categoría de consumible. """

        return self._formato_consumible_result
