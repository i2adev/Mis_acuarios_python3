...
from ModuloMaestro.Model.search_commands import SearchCmd
...
class Paginator:
    ...
    def configure_paginator(self, data_list: list[BaseEntity],
                            page: int) -> None:
        """
        Configura el paginador.
    ...
    def map_entity_list(self, rows) -> list[BaseEntity]:
        ...
        if self.procedure == "VISTA_TIPOS_FILTRO":
            ...
            data_list = [ControladorTemperaturaEntity(
                ...
                modelo=f["MODELO"],
                numero_serie=f["NUMERO_SERIE"],
                temperatura_minima=f["T_MIN"],
                temperatura_maxima=f["T_MAX"],
                consumo=f["CONSUMO"],
                fecha_alta=f["FECHA_ALTA"],
                fecha_baja=f["FECHA_BAJA"],
                motivo_baja=f["MOTIVO_BAJA"],
