...
class SearchCmd:
    ...
    SELECT ID, NUM, REINO, FILO, CLASE, ORDEN, FAMILIA, GENERO, ESPECIE,
    ...
    SELECT E.ID_ESPECIE AS ID,
           ROW_NUMBER() OVER (ORDER BY E.NOMBRE_CIENTIFICO) AS NUM,
           E.REINO AS REINO,
           E.FILO AS FILO,
           E.CLASE AS CLASE,
           E.ORDEN AS ORDEN,
    ...
    SEARCH_REQUERIMIENTO_ILUMINACION = """
    SELECT ID, NUM, REQUERIMIENTO_ILUMINACION, DESCRIPCION
    FROM
    (
        SELECT ID_REQUERIMIENTO_ILUMINACION AS ID,
               ...
               DESCRIPCION AS DESCRIPCION,
               UPPER(
                   IFNULL(REQUERIMIENTO_ILUMINACION ,'') ||
                   IFNULL(DESCRIPCION,'')
               ) AS FIELD
    ...
    SEARCH_ESPECIE_VEGETAL = """
    SELECT ID, NUM, REINO, DIVISION, CLASE, ORDEN, FAMILIA, GENERO, ESPECIE,
           NOMBRE_CIENTIFICO, NOMBRE_COMUN, PH, KH, GH, TEMPERATURA, ORIGEN,
           POSICION_ACUARIO, REQUERIMIENTO_ILUMINACION, REQUERIMIENTO_CO2,
           TASA_CRECIMIENTO, DIFICULTAD, OBSERVACIONES
    FROM
    (
    SELECT E.ID_ESPECIE AS ID,
           ...
           UPPER(
               IFNULL(E.REINO, '') || IFNULL(E.DIVISION, '') || IFNULL(E.CLASE, '') ||
               IFNULL(E.ORDEN, '') || IFNULL(E.FAMILIA, '') || IFNULL(E.GENERO, '') ||
               IFNULL(E.ESPECIE, '') || IFNULL(E.NOMBRE_CIENTIFICO, '') || IFNULL(E.NOMBRE_COMUN, '') ||
               IFNULL(
                   CASE
                       WHEN E.PH_MINIMO IS NULL AND E.PH_MAXIMO IS NULL THEN NULL
                       WHEN E.PH_MINIMO IS NOT NULL AND E.PH_MAXIMO IS NULL THEN 'Mín. ' || E.PH_MINIMO
               ...
               IFNULL(       
                   CASE
                       ...
                       ELSE E.TEMPERATURA_MIN || ' - ' || E.TEMPERATURA_MAX
                   END, '') ||
               IFNULL(E.ORIGEN, '') || IFNULL(P.POSICION, '') || IFNULL(I.REQUERIMIENTO_ILUMINACION, '') ||
               IFNULL(C.REQUERIMIENTO_CO2, '') || IFNULL(T.CRECIMIENTO, '') || IFNULL(D.DIFICULTAD_PLANTA, '') ||
               IFNULL(E.OBSERVACIONES, '')
           ) AS FIELD
    ...
    SEARCH_TASA_CRECIMIENTO = """
    SELECT ID, NUM, CRECIMIENTO, DESCRIPCION
    FROM
    (
        SELECT ID_TASA_CRECIMIENTO AS ID,
               ...
               DESCRIPCION AS DESCRIPCION,
               UPPER(
                   IFNULL(CRECIMIENTO, '') || IFNULL(DESCRIPCION, '')
               ) AS FIELD
