...
Fecha:  29/03/2026
Comentarios:
    Controlador del formulario maestro de la posición de la planta en el 
    acuario.
"""
...
from ModuloMaestro.Model.TableModel.posicion_planta_acuario_table_model import \
    PosicionPlantaAcuarioTableModel
from ModuloMaestro.Views.Masters.posicion_pnalta_acuario_view import \
    PosicionPlantaAcuarioView
from ModuloMaestro.Views.table_menu_contextual import TableMenuContextual


class PosicionPlantaAcuarioMasterController(PosicionPlantaAcuarioController):
    """ 
    Controlador del formulario maestro de la posición de planta en el 
    acuario. 
    """

    def __init__(self, view: PosicionPlantaAcuarioView,
                 dao: PosicionPlantaAcuarioDAO,
                 mod: PosicionPlantaAcuarioEntity):
        """
        Constructor base
        :param view: Formulario maestro de comportamiento de la posición de
        la planta en el acuario
        :param dao: DAO de comportamiento de la posición de
        la planta en el acuario
        :param mod: Modelo de comportamiento de la posición de
        la planta en el acuario
        """
        ...
        self._pag = Paginator("VISTA_POSICIONES_PLANTAS_ACUARIO", 5)
    ...
    def button_filter_clicked(self):
        ...
        if self._pag.status == "FILTERED":
            ...
            self._view.button_filter.setIcon(
                QIcon(str(Path(globales.PATH_IMAGES) / "filter.png"))
            )
            self._configure_status_bar(self._pag)
    ...
    def _fill_tableview(self, table: QTableView,
                        data: list[PosicionPlantaAcuarioEntity]):
        """ Carga los datos en la tabla. """

        tv_model = PosicionPlantaAcuarioTableModel(data)
